...
// Helper function to canonicalize memory kind with device validation.
// Ensures the memory kind is valid for the first device in the list.
...
bool ComputeIsFullyReplicated(const ShardingParam& sharding_param) {
  // Check if all dimension shards are 1 (not split)
  for (auto shards : sharding_param.dim_shards()) {
    if (shards != 1) {
      return false;
    }
  }
...
// Iterates the major-to-minor Cartesian product of a Span of containers.
...
class MajorToMinorIter {
 ...
 private:
  MajorToMinorIter(absl::Span<const ContainerT> containers,
    ...
    CHECK_EQ(iters.size(), containers.size());
  }
...
std::vector<Index> GetTileIndices(absl::Span<const int64_t> dim_shards) {
  ...
  int64_t shard_count = std::accumulate(
      dim_shards.begin(), dim_shards.end(), 1LL, std::multiplies<int64_t>());
...
// ============================================================================
// Base Sharding Class Implementation
// ============================================================================

Sharding::Sharding(DeviceListRef devices, 
...
bool Sharding::operator==(const Sharding& other) const {
  ...
  // Check if they have the same partitioning scheme, memory kind, and devices
  return HasSamePartitioning(other) && 
...
// ============================================================================
// SingleDeviceSharding Implementation
// ============================================================================
...
std::unique_ptr<SingleDeviceSharding> SingleDeviceSharding::Create(
  ...
  // Create a device list with just this one device
  // Note: Assuming Client has a MakeDeviceList method
  // If not, you'll need to create DeviceList directly
  Client* client = device->client();
  std::vector<Device*> device_vec = {device};
  DeviceListRef device_list = std::make_shared<DeviceList>(device_vec);
  
  // Canonicalize memory kind for this device
  memory_kind = CanonicalizeMemoryKind(memory_kind, device_list);
...
SingleDeviceSharding::SingleDeviceSharding(DeviceListRef device_list,
                                           MemoryKind memory_kind)
    : Sharding(std::move(device_list), memory_kind,
               /*is_fully_replicated=*/true) {
  // Single device sharding is considered "fully replicated" because
  // the entire array exists on one device (replication factor = 1)
}

absl::StatusOr<Shape> SingleDeviceSharding::GetShardShape(
    const Shape& shape) const {
  // For single device, the shard shape is the entire shape
  return shape;
}
...
bool SingleDeviceSharding::HasSamePartitioning(const Sharding& other) const {
  ...
  // Two SingleDeviceShardings always have the same partitioning
  // (no partitioning, just one device)
  return dynamic_cast<const SingleDeviceSharding*>(&other) != nullptr;
}
...
SingleDeviceSharding::WithDeviceAssignment(
  ...
  // Validate: SingleDeviceSharding must have exactly one device
  ...
  // Create new sharding with updated device/memory
...
SingleDeviceSharding::Disassemble(
  ...
  Device* device = devices_->devices().front();
  
  // Check if we should include this shard based on semantics
  if (single_device_shard_semantics == SingleDeviceShardSemantics::kAllShards ||
      device->IsAddressable()) {
    result.reserve(1);
    // The shard is the entire shape, placed on this device
    result.push_back({shape, SingleDeviceSharding::Create(device, memory_kind_)});
  }
...
SingleDeviceSharding::Disassemble(
  ...
  if (single_device_shard_semantics == SingleDeviceShardSemantics::kAllShards ||
      device->IsAddressable()) {
    result.reserve(1);
    // The shard is the entire dynamic shape, placed on this device
    result.push_back(
        {dynamic_shape, SingleDeviceSharding::Create(device, memory_kind_)});
  }
...
absl::StatusOr<std::vector<IndexDomain>> SingleDeviceSharding::IndexDomains(
  ...
  if (single_device_shard_semantics == SingleDeviceShardSemantics::kAllShards ||
      device->IsAddressable()) {
    result.reserve(1);
    // The index domain is the entire shape (no slicing)
...
std::string SingleDeviceSharding::DebugString() const {
  return absl::StrFormat("SingleDeviceSharding(%s, memory_kind: %v)",
                         devices_->devices().front()->ToString(),
                         memory_kind_.DebugString());
}

void SingleDeviceSharding::Hash(absl::HashState state) const {
  // Hash the device list and memory kind
  // SingleDeviceSharding is uniquely identified by these two
  absl::HashState::combine(std::move(state), devices_, memory_kind_);
}

// ============================================================================
// OpaqueSharding Implementation
// ============================================================================
...
absl::StatusOr<std::unique_ptr<Sharding>> 
OpaqueSharding::WithDeviceAssignment(
    std::optional<DeviceListRef> devices,
    std::optional<MemoryKind> memory_kind) const {
...
// ============================================================================
// ConcreteSharding Implementation
// ============================================================================
...
ConcreteSharding::ConcreteSharding(
    ...
    std::vector<Shape> shard_shapes,
    std::optional<std::vector<IndexDomain>> index_domains)
    : Sharding(std::move(devices), memory_kind, /*is_fully_replicated=*/false),
      shape_(std::move(shape)),
      shard_shapes_(std::move(shard_shapes)),
      index_domains_(std::move(index_domains)) {
  // If all per-shard shapes are the same, cache this shape for GetShardShape().
  // Ideally, users should have used ConcreteEvenSharding for such a case,
  // but there are existing use cases that instantiate ConcreteSharding from
  // a list of per-shard shapes without checking for identical per-shard shapes.
  const auto& static_shard_shapes = std::get<std::vector<Shape>>(shard_shapes_);
  bool identical = true;
  for (size_t i = 1; i < static_shard_shapes.size(); ++i) {
...
bool ConcreteSharding::HasSamePartitioning(const Sharding& other) const {
  ...
  const auto* other_concrete_sharding =
      dynamic_cast<const ConcreteSharding*>(&other);
...
// ============================================================================
// ConcreteEvenSharding Implementation
// ============================================================================
...
ConcreteEvenSharding::ConcreteEvenSharding(DeviceListRef devices,
                                           ...
                                           Shape shard_shape,
                                           bool is_fully_replicated)
    : Sharding(std::move(devices), memory_kind, is_fully_replicated),
      shape_(std::move(shape)),
...
bool ConcreteEvenSharding::HasSamePartitioning(const Sharding& other) const {
  ...
  const auto* other_concrete_even_sharding =
      dynamic_cast<const ConcreteEvenSharding*>(&other);
...
// ============================================================================
// ShardingParamSharding Implementation
// ============================================================================
...
ShardingParamSharding::Create(ShardingParam sharding_param,
                              DeviceListRef devices, MemoryKind memory_kind) {
  memory_kind = CanonicalizeMemoryKindWithDevices(memory_kind, devices);
  int64_t device_count = std::accumulate(
      sharding_param.minor_to_major().axis_sizes.begin(),
      sharding_param.minor_to_major().axis_sizes.end(),
      1LL, std::multiplies<int64_t>());
  if (device_count != static_cast<int64_t>(devices->size())) {
    return absl::InvalidArgumentError(absl::StrFormat(
...
absl::StatusOr<Shape> ShardingParamSharding::GetShardShape(
  ...
  for (size_t i = 0; i < shape.dims().size(); ++i) {
    ...
    int64_t dim_shards = sharding_param_.dim_shards()[i];
    if (dim % dim_shards != 0) {
      return absl::InvalidArgumentError(absl::StrFormat(
  ...
  return Shape(absl::MakeSpan(dims));
}
...
bool ShardingParamSharding::HasSamePartitioning(const Sharding& other) const {
  ...
  const auto* other_sharding_param_sharding =
      dynamic_cast<const ShardingParamSharding*>(&other);
...
ShardingParamSharding::Disassemble(
    const Shape& shape,
    SingleDeviceShardSemantics single_device_shard_semantics) const {
  auto local_shape_result = GetShardShape(shape);
  if (!local_shape_result.ok()) {
    return local_shape_result.status();
  }
  Shape local_shape = std::move(local_shape_result).value();
...
absl::StatusOr<std::vector<IndexDomain>> ShardingParamSharding::IndexDomains(
  ...
  CHECK_EQ(device_to_index.size() % origins.size(), 0);
  int replication = device_to_index.size() / origins.size();

  CHECK_EQ(device_to_index.size(), devices_->size());
  std::vector<IndexDomain> result;
  if (single_device_shard_semantics == SingleDeviceShardSemantics::kAllShards) {
    result.reserve(devices_->size());
  } else {
  ...
  const absl::Span<Device* const> devices = devices_->devices();
  for (size_t i = 0; i < device_to_index.size(); ++i) {
    if (single_device_shard_semantics ==
            SingleDeviceShardSemantics::kAllShards ||
        devices[i]->IsAddressable()) {
